...
import { Button, Card, ErrorState, CardSkeleton, Modal } from "../components/ui";
...
export default function ScrutinyDashboard() {
  ...
  const loadScrutinyData = async () => {
    try {
      ...
      console.error("Error loading scrutiny data:", err);
      setError(err?.response?.data?.detail || "Failed to load scrutiny data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSyncSubmit = async (e) => {
    ...
    try {
      ...
      console.error("VTU sync failed:", err);
      setSyncMessage({
        type: "error",
        text: err?.response?.data?.detail || "Failed to sync VTU resources. Check the URLs and try again.",
      });
    } finally {
  ...
  const getScoreColor = (score) => {
    const numericScore =
      typeof score === "string" ? parseInt(score.replace("%", ""), 10) : Number(score);
    ...
    <Card>
      <Card.Header>
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold text-neutral-800">Scrutiny Dashboard</h2>
          <Button variant="primary" size="sm" onClick={loadScrutinyData}>
            Refresh
          </Button>
        </div>
      </Card.Header>
      <Card.Body>
        {loading ? (
          ...
          <ErrorState
            title="Failed to load scrutiny data"
            description={error}
            retry="Retry scrutiny data"
            onRetry={loadScrutinyData}
          />
        ) : (
          <>
            {/* VTU Sync Panel */}
              ...
              <form onSubmit={handleSyncSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleSyncChange("subject_code", e.target.value.toUpperCase())
                    }
                  />
                </div>
                ...
                <div className="flex flex-col md:col-span-3">
                  <label className="text-sm font-medium text-gray-700">
                    Model Question Paper Index URL (optional)
                  </label>
                  <input
                    type="url"
                    ...
                    onChange={(e) =>
                      handleSyncChange("question_index_url", e.target.value)
                    }
                  />
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-yellow-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-yellow-600">
                    {summary.papers_needing_review}
                  </div>
                  <div className="text-sm text-gray-600">Need Review</div>
                </div>
                <div className="bg-red-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-red-600">
                    {summary.plagiarism_issues}
                  </div>
            ...
            {summary && summary.quality_distribution && (
              <div className="mb-6">
                ...
                <div className="grid grid-cols-4 gap-4">
                  {Object.entries(summary.quality_distribution).map(
                    ([quality, count]) => (
                  ...
                  {results.length === 0 ? (
                    ...
                    results.map((result) => (
                      <tr key={result.id} className="hover:bg-gray-50">
                        ...
                        <td className="border border-gray-300 p-2">
                          <span
                            className={`px-2 py-1 rounded text-xs ${getQualityColor(
      ...
      {/* Detail Modal — fixed outside Card to avoid overflow clipping */}
      <Modal
        open={showDetailModal}
        onClose={() => setShowDetailModal(false)}
        title={`Scrutiny Details — ${selectedResult?.request_info?.subject_code || ""}`}
        size="xl"
        footer={
          ...
          <div className="space-y-4">
            ...
            {selectedResult.summary?.bloom_distribution && (
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2">
                  {Object.entries(selectedResult.summary.bloom_distribution).map(
                    ([level, score]) => (
            ...
            {selectedResult.summary?.difficulty_distribution && (
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2">
                  {Object.entries(selectedResult.summary.difficulty_distribution).map(
                    ([level, count]) => (
              ...
              <div>
                ...
                <div className="mt-2 space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-bold ${
                        selectedResult.summary.plagiarism_analysis.plagiarism_score > 0.3
                          ? "text-red-600"
                          : "text-green-600"
                      }`}
                    >
                      {Math.round(
                        selectedResult.summary.plagiarism_analysis.plagiarism_score * 100
                      )}
                      %
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span>
                      {selectedResult.summary.plagiarism_analysis.duplicates?.length || 0}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Similar Questions:</span>
                    <span>
                      {selectedResult.summary.plagiarism_analysis.similar_questions?.length ||
                        0}
                    </span>
                  </div>
                </div>
            ...
            {/* Recommendations */}
            {selectedResult.summary?.recommendations &&
              selectedResult.summary.recommendations.length > 0 && (
                <div>
                ...
                <div className="mt-2 overflow-x-auto">
                  <table className="w-full border border-gray-200 text-sm">
                    <thead className="bg-gray-100">
                      <tr>
                        <th className="p-2 border border-gray-200 text-left" scope="col">
                          Module
                        </th>
                        <th className="p-2 border border-gray-200 text-left" scope="col">
                          Coverage
                    ...
                    <tbody>
                      {selectedResult.summary.syllabus_alignment.module_breakdown.map(
                        (module) => (
                          <tr key={module.module_number}>
                            ...
                            <td
                              className={`p-2 border border-gray-200 ${getScoreColor(
                                module.coverage * 100
                              )}`}
                            >
                            ...
                            <td className="p-2 border border-gray-200">
                              {module.contributing_tags.length === 0 ? (
                                ...
                                module.contributing_tags.map((tag) => (
                                  <span
                                    key={tag}
                                    className="mr-2 bg-gray-200 rounded px-2 py-1 inline-block"
                                  >
                                    {tag}
                                  </span>
                                ))
                  ...
                  <div className="text-sm text-gray-600 mt-2">
                    ...
                    <span
                      className={getScoreColor(
                        selectedResult.summary.syllabus_alignment
                          .average_module_coverage * 100
                      )}
                    >
                      {Math.round(
                        (selectedResult.summary.syllabus_alignment
                          .average_module_coverage || 0) * 100
                      )}
                      %
                    </span>
                  </div>
            ...
            {selectedResult.summary?.questions &&
              selectedResult.summary.questions.length > 0 && (
                <div>
                          ...
                          <div className="flex justify-between items-start gap-3">
                            ...
                            <div
                              className={`text-sm font-semibold ${getScoreColor(
                                (question.difficulty?.score || 0) * 100
                              )}`}
                            >
